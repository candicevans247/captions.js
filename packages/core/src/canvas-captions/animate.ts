    ...
    case 'box-word-bg':
      if (current) {
        ...
        const wordBox = new Konva.Rect({
          x: current.text.x() - paddingX,
          y: current.text.y() - paddingY,
          width: current.text.width() + paddingX * 2,
          height: current.text.height() + paddingY * 2,
          fill: current.caption.highlightColor
            ? current.caption.highlightColor
            : captionsSettings.style.aplifiedWordColor,
          cornerRadius,
          listening: false,
        });
        // insert box behind the text
        current.text.parent?.add(wordBox);
        wordBox.moveToBottom();
        // ensure text stays white (fontColor) regardless of highlight
        current.text.fill(captionsSettings.style.font.fontColor);
      }
      break;
